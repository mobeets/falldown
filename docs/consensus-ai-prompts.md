...
> intracranial/clinical depth electrodes in an epilepsy-monitoring-unit patient
> (Blackrock NSP, 78 channels, 30 kHz) while they played. Spike-sorted units
> (137 units) are aligned to the choice moment (the moment the ball passes
> through one of the two holes on the choice level), with a ±2 s window per
> trial. We have ~910 trials, plus behavioral covariates per trial: which hole
> was chosen (left/right), reaction time, the greedy (1-step) vs planning
> (2-step) cost structure of each trial, and whether the two strategies agree
> or conflict on that trial.
...
> a focus on studies of prefrontal/parietal decision-making circuits.
...
>   error/conflict monitoring) — canonical papers and the metrics used.
> - Neural correlates of **model-based vs model-free / multi-step planning**
>   (hippocampus, prefrontal, dorsolateral PFC), and how "planning horizon"
...
> Please give the canonical references and concrete analysis recipes.

## Prompt 4 — Single-trial choice decoding from population spiking

> I have trial-aligned spike counts (25 ms bins, ±2 s around choice, 137 units,
> 910 trials). I want to **decode the binary choice** (left/right hole) from
> the population on single trials, and compare decoding accuracy across
> time windows around the choice. What are the established approaches and
...
>   ECoG signals and benchmark these decoders.
...
- If you want to scope to a subfield (human intracranial, macaque, rodent,
  modeling), add one line: "Restrict to [human iEEG / nonhuman primate /
