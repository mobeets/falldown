...
def evaluate_strategy_model(model, dataloader, with_rt=False):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            correct += (preds == true_choices).sum().item()
...
def run_model(model_type='gated', participant_data_paths=None, num_strategies=3,
    ...
    if model_type == 'gated':
        ...
        _, acc, weights = evaluate_strategy_model(trained, test_loader)
        ...
        _, acc, weights = evaluate_strategy_model(trained, test_loader, with_rt=True)
    ...
    return trained, weights, {'accuracy': acc}
